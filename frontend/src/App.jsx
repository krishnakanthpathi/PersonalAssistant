      ...
      `<div class="mermaid-block-wrapper my-4 border border-white/5 rounded-xl bg-black/10 overflow-hidden">
        <div class="mermaid-block-header flex items-center justify-between px-3 py-1.5 bg-white/[0.02] border-b border-white/5 text-[10px] text-gray-400 font-mono">
          ...
          <div class="flex items-center gap-2">
            <button class="code-copy-btn" data-copy-chart-btn data-code="${encodeURIComponent(escapedCode)}" title="Copy Mermaid code">
...
// Helper to parse tool name from execution logs
...
function MainApp() {
  ...
  const handleSend = async (textToSend) => {
    ...
    setInspectMessageIndex(assistantMsgIndex);
                    ...
                    <div className="flex-grow overflow-y-auto p-4 flex flex-col gap-6 select-text">
                      {msg ? (
                        <>
                          ...
                          {/* Tools Taken (Executed) Section */}
                          <div className="flex flex-col gap-2.5">
                            <div className="flex items-center justify-between border-b border-white/5 pb-1">
                              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Tools Taken (Executed)</span>
                              <span className="px-2 py-0.5 rounded-full text-[8px] bg-accent-emerald/10 text-accent-emerald font-bold font-mono">
                                {Array.from(new Set(
                                  (msg.logs || [])
                                    .map(log => getToolNameFromLog(log))
                                    .filter(Boolean)
                                )).length} TAKEN
                              </span>
                            </div>
                            ...
                            {(() => {
                              const takenTools = Array.from(new Set(
                                (msg.logs || [])
                                  .map(log => getToolNameFromLog(log))
                                  .filter(Boolean)
                              ));

                              return takenTools.length > 0 ? (
                                <div className="flex flex-wrap gap-2">
                                  {takenTools.map((toolName, tIdx) => (
                                    <div key={tIdx} className="bg-accent-emerald/10 border border-accent-emerald/20 rounded-xl px-3 py-2 text-xs text-accent-emerald font-mono flex items-center gap-1.5 shadow-sm shadow-accent-emerald/5">
                                ...
                                <div className="text-xs text-gray-500 text-center py-4 border border-dashed border-white/5 rounded-xl">
                                  {isProcessing && inspectMessageIndex === index ? (
                                    ...
                                    "No tools were executed during this step."
                                  )}
                                </div>
                              );
                            })()}
